...
`node --test contact.test.js` проверяет три типа заявок, согласие, валидацию, honeypot, отказ почтового сервера и ограничение частоты. Письма при тесте не отправляются.
